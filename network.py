...
class Encoder(nn.Module):
    ...
    def forward(self, HR):
        ...
        feat_multi_attn = F.interpolate(feat, size=(32, 32), mode='bilinear')
        feat_multi_attn = feat_multi_attn.view(HR.shape[0], 512, -1)
        feat_multi_attn, _ = self.cont_attn(feat_multi_attn.permute(2, 0, 1), feat_multi_attn.permute(2, 0, 1), feat_multi_attn.permute(2, 0, 1))
        feat_multi_attn = feat_multi_attn.view(HR.shape[0], 512, 32, 32)
        feat_multi_attn = F.interpolate(feat_multi_attn, size=(feat.shape[2], feat.shape[3]), mode='bilinear')
        feat = feat + feat_multi_attn

        return feat


class P2Sv2(nn.Module):
    def __init__(self):
        super(P2Sv2, self).__init__()
        ...
        self.confidence = nn.Conv2d(64, 3, 3, 1, 1)
        self.reassemble = nn.Conv2d(64, 3, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        out = self.reassemble(h) * score
        heat_map = out.clone()
        heat_map[::] -= torch.min(out[::])
        heat_map[::] /= torch.max(out[::])
        out = x * heat_map
        out = out[:, 0:1, :, :] * 0.299 + out[:, 1:2, :, :] * 0.587 + out[:, 2:3, :, :] * 0.114
        return out, heat_map
...
class Vgg19(nn.Module):
    ...
    def forward(self, x):
        ...
        out = vgg_outputs(h_relu1_1, h_relu2_1, h_relu3_1)
...
class discriminator_v2(nn.Module):
    ...
    def encode(self, x):
        ...
        feat = torch.cat(
            (feat1.view(feat1.shape[0], -1), feat2.view(feat2.shape[0], -1), feat3.view(feat3.shape[0], -1)), 1)
        return feat, out3
